...
def create_contract():
    ...
    try:
        ...
        return APIResponse.success(result, "合同创建成功", 201)
...
def terminate_contract(contract_id: int):
    ...
    try:
        ...
        current_app.logger.info(f"用户 {g.username} 终止了合同 {contract.contract_no}")
        
        return APIResponse.success(contract.to_dict(), "合同终止成功")
...
def get_contract_stats():
    ...
    try:
        ...
        if landlord_id:
            ...
            query = query.filter(Contract.landlord_id == g.user_id)
