...
- **唯一约束迁移脚本**: `app/migrations/add_unique_constraints.py`
...
为以下字段添加数据库级别的唯一约束，确保数据完整性：

| 表名 | 字段名 | 说明 |
|------|--------|------|
| contracts | contract_no | 租赁合同编号 |
| payments | payment_no | 支付编号 |
| landlord_contracts | contract_no | 承包合同编号 |

### 使用方法

```bash
python app/migrations/add_unique_constraints.py
```

### 迁移流程

1. **检查表是否存在** - 确认目标表已在数据库中创建
2. **检查现有约束** - 检测是否已存在唯一约束，避免重复添加
3. **检查重复值** - 在添加约束前检查数据是否存在重复
4. **添加约束** - 通过重建表的方式添加唯一约束（SQLite 特性）

### 注意事项

- SQLite 不支持 `ALTER TABLE ADD CONSTRAINT`，因此采用重建表的方式
- 迁移前会自动检查重复值，如存在重复会跳过并提示
- 迁移过程会保留所有现有数据和索引
- **建议在执行前备份数据库**
...
----------------------------------------------------------------------
  正在为 contracts.contract_no 添加唯一约束...
  ✓ contracts.contract_no 唯一约束添加成功

[支付编号] payments.payment_no
----------------------------------------------------------------------
  ✓ payments.payment_no 唯一约束添加成功

[承包合同编号] landlord_contracts.contract_no
----------------------------------------------------------------------
  ✓ landlord_contracts.contract_no 唯一约束添加成功
